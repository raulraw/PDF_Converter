 ...
 * include config file.
 */
require_once('xml2pdf/Xml2Pdf.config.php');

/**
 * include the class Pdf.
 */
require_once('xml2pdf/main/Pdf.php');
...
Class Xml2Pdf {
     ...
     * @return void
     */
   function _parseOpeningTag($parser, $tag, $tagProperties) {
     ...
     * @return void
     */
   function _parseClosingTag($parser, $tag) {
        if(isset($this->_pdf->styles[$tag])) {
            $this->_parseContent($parser, '</'.$tag.'>');
            return;
        }
     ...
     * @return void
     */
     function _parseContent($parser, $content) {
        $tagObject = array_pop($this->_tagStack);
